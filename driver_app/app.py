"""
Suly Transit – Driver Portal
• No auth — driver enters plate number + name + line, press Go
• Streams GPS to Supabase live_bus_data every ~2 seconds (watchPosition)
• Also writes every point to bus_location_history
• Fullscreen Leaflet map shows driver's own trail in real time
• Stop button ends the session and removes live row
"""
...
async function onGPS(pos) {{
  ...
  // Upsert live row (matches live_bus_data schema)
  ...
  // Insert history row (matches bus_location_history schema)
